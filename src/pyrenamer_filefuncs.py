...
def set_stop(stop):
    ...
    global STOP
    STOP = stop
    if dbg.DEBUG: dbg.print_dbg("set_stop: %s" % STOP)


def get_stop():
    if dbg.DEBUG: dbg.print_dbg("get_stop: %s" % STOP)
    return STOP
...
def escape_pattern(pattern):
    ...
    if dbg.DEBUG: dbg.print_dbg("espace_pattern: %s" % pattern)
    return pattern
...
def get_file_listing(dir, mode, pattern=None):
    ...
    if  pattern == (None or ''):
        if dbg.DEBUG: dbg.print_dbg("get_file_listing: start dircache.listdir: %s" % dir)
        listaux = dircache.listdir(dir)
        if dbg.DEBUG: dbg.print_dbg("get_file_listing: stop dircache.listdir: %s" % dir)
    else:
        if dir != '/': dir += '/'
        dir = escape_pattern(dir + pattern)
        if dbg.DEBUG: dbg.print_dbg("get_file_listing: start glob.glob: %s" % dir)
        listaux = glob.glob(dir)
        if dbg.DEBUG: dbg.print_dbg("get_file_listing: stop glob.glob: %s" % dir)

    if dbg.DEBUG: dbg.print_dbg("get_file_listing: start adding elements to list")
    ...
    for elem in listaux:
        ...
        if mode == 0:
            ...
            if not os.path.isdir(os.path.join(dir,elem)):
                ...
                if dbg.DEBUG: dbg.print_dbg("get_file_listing: file: adding %s" % elem)
        elif mode == 1:
            # Get directories
            if os.path.isdir(os.path.join(dir,elem)):
                filelist.append([os.path.basename(elem),os.path.join(dir,elem)])
                if dbg.DEBUG: dbg.print_dbg("get_file_listing: dirs: adding %s" % elem)
        elif mode == 2:
            # Get files and directories
            filelist.append([os.path.basename(elem),os.path.join(dir,elem)])
            if dbg.DEBUG: dbg.print_dbg("get_file_listing: files/dirs: adding %s" % elem)
        else:
            # Get files
            if not os.path.isdir(os.path.join(dir,elem)):
                filelist.append([os.path.basename(elem),os.path.join(dir,elem)])
                if dbg.DEBUG: dbg.print_dbg("get_file_listing: elsefiles: adding %s" % elem)

    if dbg.DEBUG: dbg.print_dbg("get_file_listing: stop adding elements to list")
    return filelist
...
def get_file_listing_recursive(dir, mode, pattern=None):
    ...
    # Get subdirs
    if dbg.DEBUG: dbg.print_dbg("get_file_listing_recursive: start: %s" % dir)
    for root, dirs, files in os.walk(dir, topdown=False):
        ...
        for directory in dirs:
            ...
            for i in elem:
                if STOP: return filelist
                filelist.append(i)
                if dbg.DEBUG: dbg.print_dbg("get_file_listing_recursive: adding: %s" % i)

    # Get root directory files
    if dbg.DEBUG: dbg.print_dbg("get_file_listing_recursive: start root get_file_listing: %s" % dir)
    list = get_file_listing(dir, mode, pattern)
    if dbg.DEBUG: dbg.print_dbg("get_file_listing_recursive: stop root get_file_listing: %s" % dir)
    for i in list:
        filelist.append(i)
        if dbg.DEBUG: dbg.print_dbg("get_file_listing_recursive: root adding: %s" % i)

    if dbg.DEBUG: dbg.print_dbg("get_file_listing_recursive: stop: %s" % dir)
    return filelist
...
def get_dir_listing(dir):
    ...
    if dbg.DEBUG: dbg.print_dbg("get_dir_listing: start: %s" % dir)
    if dbg.DEBUG: dbg.print_dbg("get_dir_listing: start dircache.listdir: %s" % dir)
    listaux = dircache.listdir(dir)
    if dbg.DEBUG: dbg.print_dbg("get_dir_listing: stop dircache.listdir: %s" % dir)
    listaux.sort(key=str.lower)
    for elem in listaux:
        if STOP: return dirlist
        if os.path.isdir(os.path.join(dir,elem)): dirlist.append([os.path.basename(elem),os.path.join(dir,elem)])
        if dbg.DEBUG: dbg.print_dbg("get_dir_listing: adding  %s" % elem)

    if dbg.DEBUG: dbg.print_dbg("get_dir_listing: stop: %s" % dir)
    return dirlist
